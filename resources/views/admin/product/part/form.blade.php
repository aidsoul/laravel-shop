...
<div class="form-group">
    <textarea class="form-control" name="short_content" placeholder="Короткое описание"
    maxlength="500" rows="4">{{ old('short_content') ?? $product->short_content ?? '' }}</textarea>
</div>
<div class="form-group">
...
<label class="form-check-label">
    <p>При выборе больше одного изображения, будет создана галерея-изображений.</p>
    <i>Первое изображение всегда будет главным для товара</i>
</label>
     <input type="file" class="form-control-file" multiple name="images[]" accept="image/png, image/jpeg">
    </div>
...
@if($gallery > 1)
    <div class="form-group form-check">
        ...
        <label class="form-check-label" for="remove">
            Удалить загруженные изображения?
            <br>
            <i>Рекомендуется при выборе новых изображений</i>
        </label>
    </div>
@else 
@isset($product->image)
